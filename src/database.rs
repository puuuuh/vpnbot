...
impl Database {
    ...
    pub async fn peer(&self, addr: Ipv4Addr) -> Result<Option<Peer>> {
        let t = &addr.octets()[..];
        let t = sqlx::query!("SELECT * FROM peers WHERE ip = $1", t)
    ...
    pub async fn full_peer(&self, addr: Ipv4Addr) -> Result<Option<FullPeer>> {
        let t: u32 = addr.into();
        let t = sqlx::query!("SELECT settings.double_vpn, peers.ip, peers.public_key FROM peers LEFT JOIN settings ON peers.ip = settings.ip WHERE peers.ip = $1", t)
            .fetch_optional(&self.pool)
            .await?;
    ...
    pub async fn full_peers(&self) -> Result<Vec<FullPeer>> {
        let t = sqlx::query!("SELECT settings.double_vpn, peers.ip, peers.public_key FROM peers LEFT JOIN settings ON peers.ip = settings.ip")
            .fetch_all(&self.pool)
            .await?;
    ...
    pub async fn update_peer_settings(&self, ip: Ipv4Addr, settings: PeerSettings) -> Result<()> {
        let ip: u32 = ip.into();
        sqlx::query!("INSERT INTO settings VALUES($1, $2) ON CONFLICT(ip) DO UPDATE SET double_vpn = excluded.double_vpn", ip, settings.double_vpn)
            .execute(&self.pool)
            .await?;
        Ok(())
    }
    ...
    pub async fn peers_count(&self) -> Result<usize> {
        Ok(sqlx::query!("SELECT COUNT(*) as count FROM peers")
            .fetch_one(&self.pool)
            .await?
            .count as _)
    }
